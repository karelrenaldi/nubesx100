...
#pragma omp parallel for private(i)
    ...
    //         print_array((arr_mat[i].mat[j]), target_col);
    //     }
	// 	printf("range:\n");
	// 	print_array(&arr_range[i], num_targets);
    // }
	...
	printf(" Convolution matrix in %f seconds \n", run_time);
    printf(" %d threads\n",omp_get_max_threads());
    dN = (double) target_row*target_col;
    mflops = 2.0 * dN * dN * dN/(1000000.0* run_time);

    printf(" Convolution matrix at %f mflops\n", mflops);
